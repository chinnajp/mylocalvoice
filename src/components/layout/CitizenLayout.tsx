...
import { Home, FolderOpen, Bell, UserRound, Languages, TreePine } from 'lucide-react'
...
export function CitizenLayout() {
  ...
  // Keep light emerald shell for My Reports / Profile / Alerts
  useEffect(() => {
    if (!languageChosen || !citizen) return
    const root = document.documentElement
    const body = document.body
    root.classList.remove('dark')
    body.classList.remove('dark-theme')
  }, [languageChosen, citizen])
  ...
  return (
    <div className="min-h-dvh bg-emerald-50 text-emerald-950 flex flex-col">
      ...
      <main className={cn('flex-1 pb-24', !isHome && 'pt-4')}>
        <Outlet />
      </main>

      <nav className="fixed bottom-0 inset-x-0 z-40 border-t border-emerald-100 bg-white/95 backdrop-blur safe-bottom">
        <div className="max-w-lg mx-auto grid grid-cols-4 px-1 pt-1 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
